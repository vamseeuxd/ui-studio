...
export class ManageUifGroupsService {
  ...
  private uifComponentGroupList: UifComponentGroupSourceModel = {
    data: [],
    idField: 'id',
    labelField: 'name'
  };
  private uifComponentGroupsSubject = new BehaviorSubject<UifComponentGroupSourceModel>(this.uifComponentGroupList);
  public uifComponentGroupsAction$ = this.uifComponentGroupsSubject.asObservable();

  constructor() {
    window.localStorage.setItem('uifComponentGroups', JSON.stringify(this.mockData));
  ...
  public getComponentById(uifComponentId: string): UifComponentConfigInterface {
    return this.uifComponentList.find(component => (component.id === uifComponentId));
  }
  ...
  getGroups() {
    ...
    if (uifComponentGroupsStr && uifComponentGroupsStr.trim().length > 0) {
      this.uifComponentGroupList.data = [];
  ...
  addGroup(newGroupName: string) {
    const newGroup: UifComponentGroupModel = {id: new Date().getTime().toString(), name: newGroupName, components: [], isOpen: false};
  ...
  moveUp(id: string) {
    ...
    moveItem(index, index - 1, this.uifComponentGroupList.data);
  }
